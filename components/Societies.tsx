...
import { Plus, Building2, MapPin, Phone, Mail, CheckCircle, FileText, Trash2, Edit, Copy, ArrowRight, ShieldCheck, Info, X, Users } from 'lucide-react';
              ...
              <div className="bg-white rounded-3xl p-8 w-full max-w-xl shadow-2xl animate-in zoom-in duration-300 relative overflow-hidden">
                  <div className="flex justify-between items-center mb-8 relative z-10">
                      <div>
                          ...
                          <p className="text-sm text-slate-500 mt-1">Replicate Accounting Heads and Member Lists (with Opening Balances).</p>
                      </div>
                  ...
                  <form onSubmit={handleProcessCopy} className="space-y-8 relative z-10">
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 items-center">
                          <div className="space-y-2">
                              <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest px-1">Source Society (Copy FROM)</label>
                          ...
                          <div className="space-y-2">
                              <label className="block text-[10px] font-black text-indigo-400 uppercase tracking-widest px-1">Target Society (Paste TO)</label>
                      ...
                      <div className="bg-slate-50 p-6 rounded-3xl border border-slate-100 space-y-4">
                          <label className="flex items-center gap-4 cursor-pointer p-4 bg-white rounded-2xl border border-slate-200 hover:border-indigo-300 transition-all">
                              ...
                              <div className="flex-1"><p className="font-bold text-slate-800 text-sm">Master Account Heads (Chart of Accounts)</p></div>
                          ...
                          <label className="flex items-center gap-4 cursor-pointer p-4 bg-white rounded-2xl border border-slate-200 hover:border-indigo-300 transition-all">
                              ...
                              <div className="flex-1"><p className="font-bold text-slate-800 text-sm">Member Master (Including Opening Balances)</p></div>
                      ...
                      <div className="bg-amber-50 p-4 rounded-2xl border border-amber-100 flex gap-3">
                          ...
                          <p className="text-[10px] text-amber-800 leading-relaxed font-bold uppercase tracking-tight">Warning: This replicates all members and their current opening balances to the target society.</p>
                      </div>
                      <div className="flex justify-end gap-3 pt-6 border-t border-slate-100">
                          ...
                          <button type="submit" className="px-12 py-3 bg-indigo-600 text-white rounded-2xl font-black shadow-xl hover:bg-indigo-700 transition-all flex items-center gap-2 active:scale-95"><Copy size={20} /> Process Replicate</button>
                      </div>
                  </form>
              </div>
          </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-8">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  <div className="col-span-2">
                    ...
                    <input type="text" required value={newSociety.name || ''} className="w-full p-4 bg-slate-50 border border-slate-200 rounded-2xl focus:ring-4 focus:ring-indigo-50 outline-none font-black text-slate-800" onChange={e => setNewSociety({...newSociety, name: e.target.value})}/>
                  </div>
                  <div className="col-span-2">
                    <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2 px-1">Address *</label>
                    <textarea required rows={2} value={newSociety.address || ''} className="w-full p-4 bg-slate-50 border border-slate-200 rounded-2xl focus:ring-4 focus:ring-indigo-50 outline-none font-medium text-slate-700" onChange={e => setNewSociety({...newSociety, address: e.target.value})}/>
                  </div>
              </div>
              <div className="flex justify-end gap-4 pt-6 border-t border-slate-100 mt-4">
                <button type="button" onClick={() => setIsModalOpen(false)} className="px-8 py-3 text-slate-500 font-bold">Discard</button>
                <button type="submit" className="px-12 py-3 bg-indigo-600 text-white rounded-2xl font-black shadow-xl shadow-indigo-100 hover:bg-indigo-700 transition-all">{editingId ? 'Update Official Registry' : 'Complete Registration'}</button>
              </div>
            </form>
          </div>
        </div>
